...
test('speech ducks work without changing the volume preference',()=>{const {audio}=fixture();audio.unlock();audio.setVolume(.8);audio.setDucked(true);assert.equal(audio.volume,.8);assert.equal(audio.workGain.gain.value,.18);audio.setDucked(false);assert.equal(audio.workGain.gain.value,.45);audio.stopAll();});
...
for (const [page,name,kind] of [['water_minigame','water'],['fertilizer_minigame','soil'],['cooking','cut','cut'],['cooking','cook','stir']]) {
 test(`${name}: repeated correct overlays preserve the same work source and gain`,async()=>{
  ...
  for(let i=0;i<6;i++) {
   advance(100);audio.handle(i%2 ? undefined : state,[{kind:'correct',count:1}]);await flush();
